...
export default function ProjectCard({ title, description, tags, github, thumbnail }: ProjectCardProps) {
  return (
    <div className="border rounded-lg shadow p-4 sm:p-6 bg-white dark:bg-gray-800 transition-colors duration-300 hover:shadow-xl hover:-translate-y-1 transform group">
      {thumbnail && (
      ...
      <a href={github} target="_blank" rel="noopener noreferrer" className="block w-full text-center text-sm text-blue-500 dark:text-blue-300 underline py-3 rounded-lg mt-2 min-h-[44px] focus:outline-none focus:ring-2 focus:ring-primary">
        View on GitHub
      </a>
    </div>
  );
